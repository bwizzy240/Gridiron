export default async function handler(req, res) {
  ...
  try {
    const r = await fetch(
      'https://api.elections.kalshi.com/trade-api/v2/events?series_ticker=KXNFLSPREAD&status=open&with_nested_markets=true&limit=200'
    );
    ...
    const match = events.find(e =>
      e.event_ticker && e.event_ticker.startsWith('KXNFLSPREAD-') && e.event_ticker.endsWith(suffix)
    );
